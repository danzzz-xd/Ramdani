...
let handler = async (m, { conn, usedPrefix, DevMode }) => { 
    try { 
        ...
        if (global.DATABASE._data.users[m.sender].healt > 79) {
            ...
            let str = ` 
💗Nyawa mu berkurang -${healt * 1} karena Kamu telah berpetualang sampai ${pickRandom(['Jepang✈️', 'Korea🎡', 'Bali🏢', 'Amerika🗽', 'Israel🛩️', 'German🛫', 'Finlandia🛶', 'Ke bawa dunia mimpi🖤', 'Ujung dunia🗺️', 'Mars🛸', 'Bulan🌑', 'Pluto🪐', 'Matahari☀️', 'Hatinya dia❤️', '...'])} dan mendapatkan
*♻️exp:* ${exp} 
*💵uang:* ${uang}
*🗑️sampah:* ${sampah} Bonus : ${potion == 0 ? '' : '\n*🕯️Potion:* ' + potion + ''}${diamond == 0 ? '' : '\n*💎diamond:* ' + diamond + ''}${common == 0 ? '' : '\n*🎟️common crate:* ' + common + ''}${uncommon == 0 ? '' : '\n*💌uncommon crate:* ' + uncommon + ''}
`.trim()
...
handler.help = ['adventure', 'petualang', 'mulung', 'work']
handler.tags = ['rpg']
