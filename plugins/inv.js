...
let pasir = global.DATABASE.data.users[m.sender].pasir 
    ...
    let str = `
📦 Inventory Milik *${name.vnmae || name.notify || name.name || ('+' + name.jid.split`@`[0])}*
${readMore}
🪀 Health: *${healt}*
...
\n
*achievement*
1.Top ♻️ level *${userslevel.indexOf(m.sender) + 1}* dari *${userslevel.length}*
...
7.Top 🏴‍☠️ Mythic *${usersmythic.indexOf(m.sender) + 1}* dari *${usersmythic.length}*
8.Top 🐥 Legendary *${userslegendary.indexOf(m.sender) + 1}* dari *${userslegendary.length}*
9.Top 🗑️ Sampah *${userssampah.indexOf(m.sender) + 1}* dari *${userssampah.length}*

10. *DI TAS PENAMBANGAN MU* 👷
🎒 Stone      = [ ${stone} ] 
🎒 Iron          = [ ${iron} ] 
🎒 Gold         = [ ${gold} ] 
🎒 Emerald   = [ ${emerald} ] 
