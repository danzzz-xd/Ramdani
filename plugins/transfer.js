            ...
            case 'money':
                if (global.DATABASE._data.users[m.sender].money >= count * 1) {
                    try {
                        ...
                        conn.reply(m.chat, `âœ …Berhasil mentransfer money ðŸ’µ sebesar ${count}`.trim(), m)
                    } catch (e) {
                        global.DATABASE._data.users[m.sender].money += count * 1
                ...
                } else conn.reply(m.chat, `ðŸ’°Uang kamu tidak mencukupi untuk mentransfer Money ðŸ’µ sebesar ${count}`.trim(), m)
                break
            case 'potion':
